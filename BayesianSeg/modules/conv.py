...
class _BayesConvNd(Module):
    ...
    def reset_parameters(self):
        ...
        # self.weight_mu.data.normal_(self.prior_mu, self.prior_sigma)
        # self.weight_sigma.data.fill_(self.weight_mu.data.std())

        # if self.bias :
        #     self.bias_mu.data.normal_(self.prior_mu, self.prior_sigma)
        #     self.bias_sigma.data.fill_(self.bias_mu.data.std())


    def freeze(self) :
        self.weight_eps = torch.randn_like(self.weight_sigma.log())
        if self.bias :
            self.bias_eps = torch.randn_like(self.bias_sigma.log())
...
class BayesConvTransposed2d(_BayesConvNd):
    r"""
    Applies Bayesian Convolution for 2D inputs
    ...
    def __init__(self, prior_mu, prior_sigma, in_channels, out_channels, 
        ...
        padding = _pair(padding)
        dilation = _pair(dilation)
        super(BayesConvTransposed2d, self).__init__(
            prior_mu, prior_sigma, in_channels, out_channels, kernel_size, stride, 
            padding, dilation, True, _pair(0), groups, bias, padding_mode)
    ...
    def forward(self, input):
        ...
        if self.padding_mode == 'circular':
            expanded_padding = ((self.padding[1] + 1) // 2, self.padding[1] // 2,
                                (self.padding[0] + 1) // 2, self.padding[0] // 2)
            return F.conv_transpose2d(F.pad(input, expanded_padding, mode='circular'),
                            weight, bias, self.stride,
                            _pair(0), self.dilation, self.groups)
